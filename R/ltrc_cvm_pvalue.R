...
#' @param holes How to deal with holes when estimating the distribution function of the
...
#' @param plot.curves Logical, default to FALSE. If TRUE, the \eqn{k} samples to be compared
#' are depicted.
#' @param keep.boot Logical, default to FALSE. If TRUE, the function returns the statistics
#' computed from the bootstrap resamples.
#'
#' @return A list with the following:
#' * `statistic`: Evaluation of the Cramér--von Mises statistic on the samples.
#' * `pvalue`: \eqn{p}-value approximated by bootstrap.
#' * `curves`: A list with the estimators of the \eqn{k} distribution functions to
#' be compared.
#' * `groups`: Group indicator
#' * `stat.boot`: Evaluation of the Cramér--von Mises statistic on the bootstrap resamples.
#' Only included in the output if `keep.boot = TRUE`.
#'
#' @details
...
ltrc.pv.cvm <- function(p.sample, weights = NULL, B = 500,
  ...
  stat.0 <- ltrc.cvm(p.sample, weights = weights, holes = holes.internal,
  ...
  for (j in seq_len(K)) {
    ...
    if (length(QQ[[j]]$fail.time) == 0L) {
      QQ[[j]]$fail.time <- Inf
      QQ[[j]]$prob <- 1
      warning(paste("Group", j, "has no residual censoring times."))
  ...
  for (b in seq_len(B)) {
    ...
    stat.boot[b] <- ltrc.cvm(resample.df, weights = weights,
