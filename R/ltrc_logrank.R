...
#' @param p,q Nonnegative weight parameters. If one has length 1 and the other
#'   is a vector, the scalar is recycled.
#' @param holes One of 'holes' (default) or 'conditional'. In contrast to other functions of this package
#' the possibility of adding 1 to the risk set is not considered.
...
ltrc.logrank <- function(p.sample, p = 0, q = 0, plot.curves = FALSE,
  ...
  if (ncol(p.sample) < 4L) {
    stop("`p.sample` must have at least 4 columns: entry, exit, status, group.")
  }
  ...
  groups <- unique(as.character(p.sample[, 4]))
  K <- length(groups)
  ...
  for (i in seq_len(K)) {
    samp_i <- p.sample[p.sample[, 4] == groups[i], , drop = FALSE]
    est_i <- ltrc.estimator(samp_i, conf.int = FALSE, holes = holes)
  ...
  ## If conditional, condition all samples above the largest hole
  if (holes == "conditional" && any(!is.na(largest.holes))) {
    t0 <- max(largest.holes, na.rm = TRUE)
    p.sample <- p.sample[p.sample[, 2] > t0, , drop = FALSE]
  }

  times <- sort(unique(p.sample[p.sample[, 3] == 1, 2]))
  D <- length(times)

  if (D == 0L) {
    stop("No observed failures remain after applying the selected hole handling.")
  }
  ...
  for (i in seq_len(K)) {
    samp <- p.sample[p.sample[, 4] == groups[i], , drop = FALSE]

    if (nrow(samp) == 0L) next

    idx <- match(samp[samp[, 3] == 1, 2], times)
    idx <- idx[!is.na(idx)]
    if (length(idx) > 0L) {
  ...
  est.pooled <- ltrc.estimator(p.sample, conf.int = FALSE, holes = holes)
  ...
  if (plot.curves) {
    ...
    plot(ltrc.estimator(p.sample[p.sample[, 4] == groups[1], , drop = FALSE],
    ...
    if (K >= 2L) {
      for (i in 2:K) {
        lines(ltrc.estimator(p.sample[p.sample[, 4] == groups[i], , drop = FALSE],
                             conf.int = FALSE, holes = holes),
              col = i + 1)
      }
    }
